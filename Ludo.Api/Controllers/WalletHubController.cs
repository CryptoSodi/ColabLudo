...
[ApiController]
[Route("api/wallet-hub")]
public class WalletHubController(
    ApiPlayerContext playerContext,
    IDbContextFactory<LudoDbContext> contextFactory) : ControllerBase
{
    [HttpGet("bonuses")]
    public async Task<ActionResult<List<WalletBonusHistoryItem>>> GetBonuses()
    {
        var player = await playerContext.GetAuthenticatedPlayerAsync(Request);
        if (player == null)
        {
            Console.WriteLine("[WalletHubApi] GetBonuses unauthorized.");
            return Unauthorized();
        }
        ...
        Console.WriteLine($"[WalletHubApi] GetBonuses completed. PlayerId={player.PlayerId}, Count={items.Count}");
        return items;
    }

    [HttpGet("deposits")]
    public async Task<ActionResult<List<WalletDepositHistoryItem>>> GetDeposits()
    {
        var player = await playerContext.GetAuthenticatedPlayerAsync(Request);
        if (player == null)
        {
            Console.WriteLine("[WalletHubApi] GetDeposits unauthorized.");
            return Unauthorized();
        }
        ...
        Console.WriteLine($"[WalletHubApi] GetDeposits completed. PlayerId={player.PlayerId}, Count={items.Count}");
        return items;
    }

    [HttpGet("withdrawals")]
    public async Task<ActionResult<List<WalletWithdrawalHistoryItem>>> GetWithdrawals()
    {
        var player = await playerContext.GetAuthenticatedPlayerAsync(Request);
        if (player == null)
        {
            Console.WriteLine("[WalletHubApi] GetWithdrawals unauthorized.");
            return Unauthorized();
        }
        ...
        Console.WriteLine($"[WalletHubApi] GetWithdrawals completed. PlayerId={player.PlayerId}, Count={items.Count}");
        return items;
    }

    [HttpGet("games")]
    public async Task<ActionResult<List<WalletGameHistoryItem>>> GetGames()
    {
        var player = await playerContext.GetAuthenticatedPlayerAsync(Request);
        if (player == null)
        {
            Console.WriteLine("[WalletHubApi] GetGames unauthorized.");
            return Unauthorized();
        }
        ...
        Console.WriteLine($"[WalletHubApi] GetGames completed. PlayerId={player.PlayerId}, Count={items.Count}");
        return items;
    }
